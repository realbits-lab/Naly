...
async function main() {
  ...
  try {
    // First check for existing source
    const existingSource = await sql`
      SELECT id FROM rss_sources LIMIT 1
    `;
    ...
    // Now insert test articles
    const insertArticlesSQL = `
      INSERT INTO rss_articles (
        source_id, title, content, full_content, link,
        published_at, categories, is_archived, guid
      ) VALUES
      ($1, 'NVIDIA Reports Record Q3 2024 Revenue of $35.08 Billion',
      ...
      ($1, 'Apple Achieves $89.5 Billion Revenue in Q4 2024',
      ...
      ($1, 'Tesla Delivers 462,890 Vehicles in Q3 2024',
    ...
    const result = await sql.unsafe(insertArticlesSQL, [sourceId]);
