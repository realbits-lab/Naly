...
async function runTests() {
  ...
  try {
    ...
    console.log(`⏳ Waiting ${SERVER_STARTUP_DELAY / 1000} seconds for server startup...`);
    ...
    const testCommand = 'dotenv --file .env.local run pnpm test __tests__/article-generation.test.ts --verbose';
...
async function checkPrerequisites() {
  ...
  try {
    ...
    console.error('❌ .env.local file not found. Please ensure environment variables are configured.');
    process.exit(1);
  }
  ...
  try {
    ...
    console.error('❌ Database configuration issue. Please check DATABASE_URL in .env.local');
    process.exit(1);
  }
