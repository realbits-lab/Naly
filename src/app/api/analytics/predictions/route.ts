	...
	try {
		...
		if (body.includeEnsemble) {
			try {
				...
				ensemblePrediction =
					await engine.getEnsemblePrediction(mockPredictions);
			} catch (error) {
	...
	try {
		...
		if (body.historicalData.length < 10) {
			const error: ApplicationError = {
				code: ErrorCode.VALIDATION_ERROR,
				message:
					"At least 10 historical data points are required for calibration",
				severity: ErrorSeverity.MEDIUM,
				metadata: {
					timestamp: new Date(),
					service: "predictions-api",
	...
	try {
		...
		if (!body.predictions || !body.outcomes) {
			const error: ApplicationError = {
				code: ErrorCode.VALIDATION_ERROR,
				message:
					'Request body must contain both "predictions" and "outcomes" arrays',
				severity: ErrorSeverity.MEDIUM,
				metadata: {
					timestamp: new Date(),
					service: "predictions-api",
