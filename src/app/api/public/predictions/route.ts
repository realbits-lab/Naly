...
const generatePredictionSchema = z.object({
	...
	modelType: z
		.enum(["neural_network", "ensemble", "transformer", "lstm"])
		.default("ensemble"),
	...
	try {
		...
		const generatePredictions = () => {
			...
			for (const ticker of mockTickers.slice(0, data.limit)) {
				...
				const horizon =
					data.horizon ||
					["1d", "1w", "1m", "3m"][Math.floor(Math.random() * 4)];
				const predictionType =
					data.type ||
					["price", "movement", "volatility"][Math.floor(Math.random() * 3)];
				...
				if (predictionType === "price") {
					...
					prediction.regime =
						predictedVol > 0.3 ? "high" : predictedVol > 0.2 ? "medium" : "low";
				}
			...
			createSuccessResponse(
				{
					...
					meta: {
						...
						disclaimer:
							"Predictions are for informational purposes only and should not be considered as financial advice. Past performance does not guarantee future results.",
						rateLimit: {
							remaining: 99,
...
export async function POST(request: NextRequest) {
	...
	try {
				...
				createErrorResponse(
					{
						code: ErrorCode.UNAUTHORIZED,
						message:
							"API key required for prediction generation. Get your API key at https://naly.ai/api-keys",
						severity: ErrorSeverity.MEDIUM,
						metadata: {
							timestamp: new Date(),
							service: "public-api",
		...
		const prediction = {
			...
			createdAt: new Date().toISOString(),
			expiresAt: new Date(
				Date.now() + getExpirationTime(requestData.horizon),
			).toISOString(),
...
function getExpirationTime(horizon: string): number {
	...
	return (
		expirationMap[horizon as keyof typeof expirationMap] || 24 * 60 * 60 * 1000
	);
}
	...
	features.forEach((feature) => {
		if (factorMap[feature]) {
			selectedFactors.push(
				factorMap[feature][
					Math.floor(Math.random() * factorMap[feature].length)
				],
			);
		}
	});
