	...
	try {
		...
		const queryParams = {
			...
			offset: searchParams.get("offset")
				...
				: 0,
			format:
				(searchParams.get("format") as "json" | "markdown" | "html") || "json",
		};
		...
		const narratives = [
			{
				...
				summary:
					"Apple's stock demonstrates robust performance as the company continues to integrate AI capabilities across its ecosystem.",
				content:
					"Apple Inc. (AAPL) has shown remarkable resilience in the current market environment, with its stock price reflecting strong investor confidence in the company's strategic direction. The integration of artificial intelligence across Apple's product lineup, particularly in the upcoming iOS updates and new hardware releases, has positioned the company favorably for sustained growth.\n\nTechnical analysis reveals a bullish pattern with strong support levels and increasing volume, indicating continued institutional interest. The company's fundamental metrics remain solid, with strong cash flow generation and a robust balance sheet providing stability in uncertain market conditions.",
			...
			},
			{
				...
				summary:
					"Recent market fluctuations have created attractive entry points for technology stocks with strong fundamentals.",
				content:
		...
		if (validationResult.data.ticker) {
			filteredNarratives = filteredNarratives.filter(
				(n) =>
					n.ticker.toLowerCase() ===
					validationResult.data.ticker!.toLowerCase(),
			);
		}

		if (validationResult.data.category) {
			filteredNarratives = filteredNarratives.filter(
				(n) => n.category === validationResult.data.category,
			);
		}

		if (validationResult.data.sentiment) {
			filteredNarratives = filteredNarratives.filter(
				(n) => n.sentiment === validationResult.data.sentiment,
			);
		}
		...
		const paginatedNarratives = filteredNarratives.slice(
			...
			validationResult.data.offset + validationResult.data.limit,
		);
...
export async function POST(request: NextRequest) {
	...
	try {
				...
				createErrorResponse(
					{
						code: ErrorCode.UNAUTHORIZED,
						message:
							"API key required for narrative generation. Get your API key at https://naly.ai/api-keys",
						severity: ErrorSeverity.MEDIUM,
						metadata: {
							timestamp: new Date(),
							service: "public-api",
		...
		const narrative = {
			...
			category:
				requestData.analysisType === "earnings"
					? "earnings"
					: "market_analysis",
