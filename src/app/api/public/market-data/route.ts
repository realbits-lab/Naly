...
const marketDataQuerySchema = z.object({
  ...
  period: z.enum(['1d', '5d', '1m', '3m', '6m', '1y', '2y', '5y', 'max']).default('1d'),
...
const multiTickerQuerySchema = z.object({
  ...
  metrics: z.array(z.enum([
    ...
    'peRatio', 'dividendYield', '52weekHigh', '52weekLow', 'beta'
    ...
    if (endpoint === 'quote') {
      ...
      return NextResponse.json(
        createErrorResponse({
          ...
          message: 'Invalid endpoint. Supported endpoints: quote, historical, multi',
          severity: ErrorSeverity.MEDIUM,
          metadata: {
            timestamp: new Date(),
            service: 'public-api',
...
async function handleHistoricalRequest(request: NextRequest, requestId: string) {
  const { searchParams } = new URL(request.url)
  const queryParams = {
  ...
  const generateHistoricalData = (ticker: string, period: string, interval: string) => {
  ...
  const historicalData = generateHistoricalData(data.ticker, data.period, data.interval)
...
async function handleMultiTickerRequest(request: NextRequest, requestId: string) {
  const { searchParams } = new URL(request.url)
  const queryParams = {
    tickers: searchParams.get('tickers') || '',
    metrics: searchParams.get('metrics')?.split(',') || ['price', 'change', 'changePercent', 'volume'],
    format: searchParams.get('format') || 'json'
  }
        ...
        metadata: {
          ...
          additionalData: {
            ...
            supportedParams: {
              ...
              metrics: ['price', 'change', 'changePercent', 'volume', 'marketCap', 'peRatio', 'dividendYield', '52weekHigh', '52weekLow', 'beta'],
              format: ['json', 'csv']
            }
          }
        },
  ...
  const data = validationResult.data
  const tickers = data.tickers.split(',').map(s => s.trim().toUpperCase()).filter(Boolean)
