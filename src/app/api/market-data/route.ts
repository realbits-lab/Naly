...
import { getFinancialDataAPI, initializeServices, createServiceConfiguration } from '@/lib/service-registry'
import { MarketDataRequest } from '@/types/services'
import { ApplicationError, ErrorCode, ErrorSeverity } from '@/types/errors'
import { createSuccessResponse, createErrorResponse, validateQueryParams } from '@/types'
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const frequency = searchParams.get('frequency') as 'realtime' | 'daily' | 'weekly' | 'monthly' | null
  ...
  try {
    ...
    for (let i = 0; i < body.requests.length; i++) {
      ...
      try {
        ...
        if (!req.ticker || !req.data_types || !req.start_date || !req.end_date) {
          results.push({
            index: i,
            success: false,
            error: 'Missing required fields: ticker, data_types, start_date, end_date'
          })
          continue
        }
    ...
    const genericError: ApplicationError = {
      code: ErrorCode.INTERNAL_SERVER_ERROR,
      message: 'An unexpected error occurred while processing batch market data request',
      severity: ErrorSeverity.HIGH,
      metadata: {
        timestamp: new Date(),
        service: 'market-data-api',
