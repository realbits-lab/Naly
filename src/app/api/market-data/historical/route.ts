...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const historicalData = await financialDataAPI.getHistoricalData(ticker.toUpperCase(), years)
    ...
    const metadata = {
      ...
      dateRange: {
        ...
        end: historicalData.length > 0 ? historicalData[historicalData.length - 1].timestamp : null
      }
    }

    return NextResponse.json(createSuccessResponse({ data: historicalData, metadata }, requestId))
