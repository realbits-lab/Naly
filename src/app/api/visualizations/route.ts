...
import { VisualizationType, ChartTheme } from '@/types/content'
  ...
  try {
    ...
    return NextResponse.json(createSuccessResponse({
      ...
      metadata: {
        ...
        hasInteractivity: visualization.interactivity.zoom || visualization.interactivity.pan,
  ...
  try {
    ...
    return NextResponse.json(createSuccessResponse({
      ...
      metadata: {
        ...
        isResponsive: dashboard.responsiveness.mobile && dashboard.responsiveness.tablet,
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const exportBuffer = await generator.exportVisualization(mockVisualization, format)

    // Set appropriate headers for download
    const headers = new Headers()
    headers.set('Content-Type', getContentType(format))
    headers.set('Content-Disposition', `attachment; filename="visualization_${visualizationId}.${format}"`)
...
function getVisualizationFeatures(type: VisualizationType): string[] {
  const baseFeatures = ['Interactive', 'Responsive', 'Accessible', 'Exportable']
