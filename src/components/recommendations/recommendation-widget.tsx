...
import { Recommendation, RecommendationType } from '@/lib/recommendations/recommendation-engine'
...
export function RecommendationWidget({
  ...
  const handleAction = async (recommendation: Recommendation, action: string) => {
    // Track interaction
    try {
      await fetch('/api/recommendations', {
        method: 'POST',
  ...
  const getRecommendationIcon = (type: RecommendationType) => {
    switch (type) {
      case 'INVESTMENT_OPPORTUNITY': return TrendingUp
      case 'PORTFOLIO_REBALANCE': return BarChart3
      case 'CONTENT_DISCOVERY': return BookOpen
      case 'RISK_ALERT': return AlertTriangle
      case 'LEARNING_RESOURCE': return Award
      case 'MARKET_INSIGHT': return Lightbulb
      case 'TRADING_STRATEGY': return Target
      case 'NEWS_ALERT': return Zap
  ...
  const getPriorityBadge = (priority: string) => {
    switch (priority) {
      case 'urgent': return { variant: 'destructive' as const, label: 'Urgent' }
      case 'high': return { variant: 'secondary' as const, label: 'High' }
                ...
                <div className="flex-1 min-w-0 space-y-2">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {recommendation.actionable && (
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => handleAction(recommendation, 'view_details')}
                  ...
                  {recommendation.metadata.expectedReturn && (
                    <div className="flex items-center space-x-2 text-xs">
                      ...
                      <span className={cn(
                        "font-medium",
                        recommendation.metadata.expectedReturn > 0 ? 'text-bull' : 'text-bear'
                      )}>
