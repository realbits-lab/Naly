	...
	const validateCronExpression = (expression: string): string | null => {
		...
		const validateField = (
			...
			if (field.includes("/")) {
				...
				if (range !== "*" && !validateField(range, name, [min, max]))
					return false;
			...
			if (field.includes(",")) {
				return field
					.split(",")
					.every((part) => validateField(part.trim(), name, [min, max]));
			}
					...
					>
													...
													<SelectItem key={value} value={value}>
														<div>
															...
															<div className="text-xs text-muted-foreground">
																{
																	jobTypeDescriptions[
																		value as keyof typeof jobTypeDescriptions
																	]
																}
															</div>
														</div>
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										...
										<FormDescription>
											Format: minute hour day month day-of-week (e.g., "0 * * *
											*" for hourly)
										</FormDescription>
						...
						<Card>
							...
							<CardContent className="text-xs space-y-2">
								...
								<p className="text-muted-foreground">
									Use * for any value, / for intervals, - for ranges, and , for
									lists
								</p>
							</CardContent>
						</Card>
						...
						<DialogFooter>
							...
							<Button
								...
								disabled={
									isSubmitting ||
									!!validateCronExpression(form.watch("cronExpression"))
								}
							>
								{isSubmitting
									? mode === "create"
