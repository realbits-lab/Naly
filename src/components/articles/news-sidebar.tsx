...
export function NewsSidebar({
  ...
  const filteredArticles = articles.filter(article => {
    ...
    const matchesCategory = selectedCategory === 'all' || article.sourceCategory === selectedCategory
    return matchesSearch && matchesCategory
  })

  const categories = ['all', ...Array.from(new Set(articles.map(a => a.sourceCategory).filter(Boolean)))]
                  ...
                  >
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {article.sentiment && (
                          <Badge size="sm" className={getSentimentColor(article.sentiment)}>
                            {article.sentiment}
                          </Badge>
                        )}
                      </div>
