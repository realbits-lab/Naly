...
export const isMarketEvent = (obj: any): obj is import('./market').MarketEvent => {
  return obj && typeof obj.id === 'string' && typeof obj.eventType === 'string'
}

export const isUserProfile = (obj: any): obj is import('./user').UserProfile => {
  return obj && typeof obj.userId === 'string' && obj.demographics && obj.preferences
}

export const isApplicationError = (obj: any): obj is import('./errors').ApplicationError => {
...
export const isPaginatedResponse = <T>(obj: any): obj is import('./common').PaginatedResponse<T> => {
  return obj && Array.isArray(obj.data) && obj.pagination && typeof obj.pagination.total === 'number'
}
...
export const validatePaginationParams = (params: any): import('./common').PaginationParams => {
...
export const validateQueryParams = (params: any): import('./common').QueryParams => {
  return {
    filters: Array.isArray(params.filters) ? params.filters : [],
    sort: Array.isArray(params.sort) ? params.sort : [],
    pagination: params.pagination ? validatePaginationParams(params.pagination) : undefined,
...
export const isValidEventType = (value: string): value is import('./market').EventType => {
  const EventType = require('./market').EventType
  return Object.values(EventType).includes(value)
}

export const isValidComplexityLevel = (value: string): value is import('./content').ComplexityLevel => {
  const ComplexityLevel = require('./content').ComplexityLevel
  return Object.values(ComplexityLevel).includes(value)
}

export const isValidRiskTolerance = (value: string): value is import('./user').RiskTolerance => {
  const RiskTolerance = require('./user').RiskTolerance
  return Object.values(RiskTolerance).includes(value)
}

export const isValidTimeHorizon = (value: string): value is import('./analytics').TimeHorizon => {
