...
import { MarketEvent, MarketDataPoint, MarketDataStream, DataSource } from './market'
import { CausalAnalysis, PredictiveAnalysis, EnsemblePrediction } from './analytics'
import { IntelligentNarrative, Visualization } from './content'
import { UserProfile, Recommendation, Notification, UserInteraction } from './user'
...
export interface PredictionService {
  ...
  generatePrediction(event: MarketEvent, context: any): Promise<PredictiveAnalysis>
  getEnsemblePrediction(predictions: any[]): Promise<EnsemblePrediction>
  calibrateModels(historicalData: any[]): Promise<void>
  evaluatePredictionAccuracy(predictions: any[], outcomes: any[]): Promise<number>
}
...
export interface NarrativeService {
  ...
  generateNarrative(event: MarketEvent, analysis: CausalAnalysis, prediction: PredictiveAnalysis): Promise<IntelligentNarrative>
  adaptNarrative(narrative: IntelligentNarrative, userProfile: UserProfile): Promise<IntelligentNarrative>
...
export interface VisualizationService {
  ...
  generateVisualization(data: any, type: string, config?: any): Promise<Visualization>
  createDashboard(visualizations: Visualization[]): Promise<any>
  exportVisualization(visualization: Visualization, format: string): Promise<Buffer>
}
...
export interface CommunityService {
  ...
  createDiscussion(userId: string, topic: string, content: string): Promise<any>
  addComment(discussionId: string, userId: string, content: string): Promise<any>
