...
class ServiceRegistryImpl implements ServiceFactory {
  ...
  private async initializeFinancialDataAPI(): Promise<void> {
    ...
    await client.initialize({
      ...
      baseUrl: process.env.FINANCIAL_DATASETS_API_URL || 'https://api.financialdatasets.ai/v1',
...
export const initializeServices = async (config: ServiceConfiguration): Promise<void> => {
...
export const createServiceConfiguration = (environment: 'development' | 'staging' | 'production' | 'test'): ServiceConfiguration => {
...
export const checkServiceHealth = async (): Promise<{
  ...
  try {
    ...
    const anyHealthy = Object.values(results).some(r => r.status === 'healthy')
