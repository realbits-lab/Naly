...
export const assessNarrativeComplexity = (
	content: string,
...
export const formatNarrativeMetadata = (narrative: any) => ({
	...
	formattedComplexity: narrative.metadata.complexityLevel
		.toLowerCase()
		.replace("_", " "),
	formattedAudience: narrative.metadata.targetAudience
		.toLowerCase()
		.replace("_", " "),
	sentimentLabel:
		narrative.metadata.sentiment.overall > 0.1
			? "Positive"
			: narrative.metadata.sentiment.overall < -0.1
				? "Negative"
				: "Neutral",
...
export const calculateWordCount = (narrative: any): number => {
	...
	return sections.reduce((total, content) => {
		return (
			total +
			content.split(/\s+/).filter((word: string) => word.length > 0).length
		);
	}, 0);
};
...
export const calculateAverageConfidence = (narrative: any): number => {
	...
	const totalConfidence = sections.reduce(
		(sum, section) => sum + (section.confidence || 0.5),
		0,
	);
...
export const validateNarrativeStructure = (
	narrative: any,
	...
	requiredSections.forEach((sectionName) => {
		...
		if (!section) {
			...
			if (!section.content)
				issues.push(`${sectionName} section missing content`);
	...
	if (!narrative.metadata) {
		...
		if (!narrative.metadata.complexityLevel)
			issues.push("Missing complexity level");
		if (!narrative.metadata.targetAudience)
			issues.push("Missing target audience");
		if (typeof narrative.metadata.readingTime !== "number")
			issues.push("Invalid reading time");
	}
...
export const categorizeNarrative = (
	narrative: any,
): {
	category: string;
	subcategory: string;
...
export const assessContentQuality = (
	...
	const completeSections = sections.filter(
		(s) => s && s.content && s.content.length > 100,
	);
	...
	if (
		narrative.headline &&
		narrative.headline.length > 20 &&
		narrative.headline.length < 100
	) {
		score += 10;
	...
	if (
		narrative.metadata &&
		narrative.metadata.sentiment &&
		narrative.metadata.topicalTags
	) {
		score += 15;
	...
	const sectionsWithBullets = sections.filter(
		(s) => s && s.bulletPoints && s.bulletPoints.length > 0,
	);
	...
	if (
		narrative.metadata?.topicalTags &&
		narrative.metadata.topicalTags.length >= 3
	) {
		score += 15;
...
export const NARRATIVE_ERROR_MESSAGES = {
	...
	INVALID_EVENT_STRUCTURE:
		"Event must contain id, eventType, ticker, and timestamp",
