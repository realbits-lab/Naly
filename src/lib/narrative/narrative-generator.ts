...
import {
	intelligentNarratives,
	narrativeValidations,
...
export class NarrativeGenerator implements NarrativeService {
	...
	async adaptNarrative(
		...
		try {
			...
			const personalizedTemplate =
				await this.createPersonalizedTemplate(userProfile);
	...
	private async generateAllSections(
		...
		const summaryPrompt = this.createSectionPrompt(
			SectionType.SUMMARY,
			context,
		);
		...
		const explanationPrompt = this.createSectionPrompt(
			SectionType.EXPLANATION,
			context,
		);
		...
		const predictionPrompt = this.createSectionPrompt(
			SectionType.PREDICTION,
			context,
		);
		...
		sections.push({
			...
			confidence: context.prediction?.uncertainty
				? 1 - context.prediction.uncertainty.entropy / 2
				: 0.7,
			sources: [],
			visualizations: [],
		});
		...
		if (context.template?.structure.sections.includes(SectionType.DEEP_DIVE)) {
			const deepDivePrompt = this.createSectionPrompt(
				SectionType.DEEP_DIVE,
				context,
			);
...
`
		...
		const audienceGuidance = template
			? this.getAudienceGuidance(template.targetAudience, template.complexity)
			: "";
...
- Avoid jargon or explain technical terms clearly`;
	...
	private async generateHeadline(
		...
		const prompt = {
			systemPrompt:
				"You are a financial news headline writer. Create compelling, accurate headlines.",
...
Format as JSON array of strings.`,
	...
	private async calculateNarrativeMetadata(
		...
		const sentiment = await this.calculateSentimentScore(
		...
		return {
			readingTime,
			complexityLevel:
				context.template?.complexity || ComplexityLevel.INTERMEDIATE,
	...
	private async extractTopicalTags(
		event: MarketEvent,
		sections: ContentSection[],
	): Promise<string[]> {
	...
	private async generateVisualizationSpecs(
		context: NarrativeGenerationContext,
	): Promise<any[]> {
	...
	private getAudienceGuidance(
		audience: AudienceType,
		complexity: ComplexityLevel,
	): string {
		const audienceMap = {
			[AudienceType.RETAIL]:
				"Write for individual retail investors with clear, accessible language.",
			[AudienceType.PRO_AM]:
				"Write for experienced amateur investors with moderate technical detail.",
			[AudienceType.PROFESSIONAL]:
				"Write for financial professionals with technical analysis.",
			[AudienceType.INSTITUTIONAL]:
				"Write for institutional investors with sophisticated analysis.",
		};

		const complexityMap = {
			[ComplexityLevel.BASIC]:
				"Keep explanations simple and define technical terms.",
			[ComplexityLevel.INTERMEDIATE]:
				"Use moderate technical detail with context.",
			[ComplexityLevel.ADVANCED]:
				"Include sophisticated analysis and technical concepts.",
			[ComplexityLevel.EXPERT]:
				"Use advanced financial concepts and detailed analysis.",
		};
