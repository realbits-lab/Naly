...
import { users, userProfiles, userPortfolios, userSettings, userBehaviorLogs } from '@/lib/schema/users'
import { UserProfile, UserRole, AudienceType, ComplexityLevel, InvestmentExperience } from '@/types/user'
...
export interface UserService {
  ...
  logUserBehavior(userId: string, action: string, metadata?: any): Promise<void>
...
export class DatabaseUserService implements UserService {
    ...
    try {
      ...
      throw new ApplicationError({
        ...
        metadata: {
          ...
          additionalData: { email, error: error instanceof Error ? error.message : String(error) }
        },
        retryable: true
      })
    }
  ...
  async createUser(userData: CreateUserData): Promise<UserProfile> {
    try {
      ...
      await db.insert(userProfiles).values({
        ...
        experienceLevel: userData.experienceLevel || InvestmentExperience.BEGINNER,
  ...
  async updateUserProfile(userId: string, profileData: Partial<UserProfile>): Promise<UserProfile> {
    try {
      ...
      if (profileData.displayName) profileUpdate.displayName = profileData.displayName
      if (profileData.audienceType) profileUpdate.audienceType = profileData.audienceType
      if (profileData.experienceLevel) profileUpdate.experienceLevel = profileData.experienceLevel
      if (profileData.riskTolerance) profileUpdate.riskTolerance = profileData.riskTolerance
      if (profileData.preferredComplexity) profileUpdate.preferredComplexity = profileData.preferredComplexity
      ...
      if (profileData.interests) profileUpdate.interests = profileData.interests
      if (profileData.watchedTickers) profileUpdate.watchedTickers = profileData.watchedTickers
      if (profileData.notifications) profileUpdate.notifications = profileData.notifications
      if (profileData.privacySettings) profileUpdate.privacySettings = profileData.privacySettings
  ...
  async logUserBehavior(userId: string, action: string, metadata?: any): Promise<void> {
    try {
  ...
  private mapToUserProfile(data: any): UserProfile {
    ...
    return {
      ...
      experienceLevel: profile?.experienceLevel || InvestmentExperience.BEGINNER,
      riskTolerance: profile?.riskTolerance || 'MODERATE',
      preferredComplexity: profile?.preferredComplexity || ComplexityLevel.INTERMEDIATE,
