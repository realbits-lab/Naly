...
export class FinancialDataAPIClient implements FinancialDataAPIService {
  ...
  async getMarketData(request: MarketDataRequest): Promise<MarketDataPoint[]> {
    ...
    try {
      ...
      const marketDataPoints = this.transformToMarketDataPoints(rawData, request.ticker)

      // Cache for 5 minutes for most recent data, longer for historical
      const cacheTime = this.isRecentData(request.endDate) ? 5 * 60 * 1000 : 60 * 60 * 1000
      ...
      throw this.createError(
        ...
        ErrorSeverity.HIGH,
        { ticker: request.ticker, error: error instanceof Error ? error.message : String(error) }
      )
    }
  }
  ...
  async streamMarketData(tickers: string[]): Promise<MarketDataStream> {
    ...
    try {
      ...
      throw this.createError(
        ...
        ErrorSeverity.HIGH,
        { tickers, error: error instanceof Error ? error.message : String(error) }
      )
    }
  }

  async getHistoricalData(ticker: string, years: number): Promise<MarketDataPoint[]> {
  ...
  private async checkRateLimit(): Promise<void> {
    ...
    if (this.rateLimiter.requests >= this.rateLimiter.limit) {
      const resetTime = this.rateLimiter.windowStart + this.rateLimiter.windowMs
      ...
      throw this.createError(
        ...
        ErrorSeverity.HIGH,
        { resetTime, currentRequests: this.rateLimiter.requests, limit: this.rateLimiter.limit }
      )
    }
  ...
  private transformToMarketDataPoints(rawData: any, ticker: string): MarketDataPoint[] {
      ...
      try {
        return {
          ...
          metadata: {
            ...
            freshness: this.calculateFreshness(new Date(item.timestamp || item.date)),
  ...
  private mapHttpStatusToErrorCode(status: number): ErrorCode {
    switch (status) {
      ...
      case 401: return ErrorCode.API_AUTHENTICATION_ERROR
      case 403: return ErrorCode.FORBIDDEN
      case 404: return ErrorCode.NOT_FOUND
      case 429: return ErrorCode.API_RATE_LIMIT_ERROR
      case 500: return ErrorCode.API_CONNECTION_ERROR
      case 503: return ErrorCode.SERVICE_UNAVAILABLE
      default: return ErrorCode.API_CONNECTION_ERROR
    }
  }
  ...
  private createError(
    ...
    return {
      ...
      retryable: code === ErrorCode.API_RATE_LIMIT_ERROR || severity === ErrorSeverity.HIGH
    }
  }
}