...
export interface CommunityService {
  ...
  checkAndAwardAchievements(userId: string, context: AchievementContext): Promise<UserAchievement[]>
  getChallenges(filters?: ChallengeFilters): Promise<Challenge[]>
  joinChallenge(userId: string, challengeId: string): Promise<UserChallengeProgress>
  updateChallengeProgress(userId: string, challengeId: string, progress: ChallengeProgressUpdate): Promise<UserChallengeProgress>
  getLeaderboards(type: string, period: string, limit?: number): Promise<LeaderboardEntry[]>
  updateUserScore(userId: string, scoreType: string, points: number): Promise<void>
...
export class CommunityServiceImpl implements CommunityService {
  async createDiscussion(params: CreateDiscussionParams): Promise<Discussion> {
    try {
      ...
      if (!title && params.content.length > 100) {
        title = await this.generateDiscussionTitle(params.content, params.relatedTicker)
      }

      const [discussion] = await db.insert(communityDiscussions).values({
  ...
  async getDiscussions(filters: DiscussionFilters): Promise<DiscussionWithMetadata[]> {
    try {
      const conditions = []
      ...
      switch (filters.sortBy) {
        case 'popular':
          orderBy = desc(sql`${communityDiscussions.upvotes} - ${communityDiscussions.downvotes}`)
          break
        case 'controversial':
          orderBy = desc(sql`${communityDiscussions.upvotes} + ${communityDiscussions.downvotes}`)
          break
  ...
  async replyToDiscussion(params: ReplyToDiscussionParams): Promise<Discussion> {
    try {
      const [reply] = await db.insert(communityDiscussions).values({
  ...
  async voteOnDiscussion(params: VoteParams): Promise<void> {
    try {
      ...
      await this.checkAndAwardAchievements(params.userId, {
        ...
        metadata: { discussionId: params.discussionId, voteType: params.voteType }
      })
    } catch (error) {
      throw new ApplicationError({
        code: ErrorCode.DATABASE_ERROR,
  ...
  async checkAndAwardAchievements(userId: string, context: AchievementContext): Promise<UserAchievement[]> {
    try {
      ...
      for (const rule of achievementRules) {
        const hasAchievement = await this.userHasAchievement(userId, rule.achievementId)
        if (!hasAchievement && await this.meetsAchievementCriteria(userId, rule)) {
  ...
  async joinChallenge(userId: string, challengeId: string): Promise<UserChallengeProgress> {
    try {
  ...
  async getLeaderboards(type: string, period: string, limit: number = 10): Promise<LeaderboardEntry[]> {
    try {
  ...
  async updateUserScore(userId: string, scoreType: string, points: number): Promise<void> {
    try {
      ...
      await db.insert(leaderboards).values({
        ...
        target: [leaderboards.userId, leaderboards.leaderboardType, leaderboards.period],
        set: {
  ...
  async getCommunityStats(): Promise<CommunityStats> {
    try {
      ...
      const topContributors = await this.getLeaderboards('community_points', 'monthly', 5)
  ...
  async getUserCommunityProfile(userId: string): Promise<UserCommunityProfile> {
    try {
      ...
      const activeChallenges = await db
        ...
        .leftJoin(challenges, eq(userChallengeProgress.challengeId, challenges.id))
        .where(
          and(
            eq(userChallengeProgress.userId, userId),
  ...
  private async generateDiscussionTitle(content: string, ticker?: string): Promise<string> {
    try {
...
Return only the title, no quotes or additional text.`
  ...
  private async meetsAchievementCriteria(userId: string, rule: any): Promise<boolean> {
  ...
  private async awardAchievement(userId: string, rule: any): Promise<UserAchievement> {
