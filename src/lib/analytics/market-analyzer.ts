...
export class MarketDataAnalyzer {
	...
	detectAnomalies(
		...
		if (priceData.length >= lookbackPeriod) {
			const volatilityAnomaly = this.detectVolatilityAnomaly(
				priceData,
				lookbackPeriod,
			);
	...
	private calculateBollingerBands(
		...
		const variance =
			recentPrices.reduce((sum, price) => sum + (price - sma) ** 2, 0) / period;
	...
	private calculateSkewness(
		...
		const n = data.length;
		const skew = data.reduce(
			(sum, val) => sum + ((val - mean) / standardDeviation) ** 3,
			0,
		);
		return skew / n;
	}
	...
	private calculateKurtosis(
		...
		const n = data.length;
		const kurt = data.reduce(
			(sum, val) => sum + ((val - mean) / standardDeviation) ** 4,
			0,
		);
	...
	private detectPriceAnomaly(
		...
		const zScore = Math.abs(
			(currentPrice - stats.mean) / stats.standardDeviation,
		);
	...
	private detectVolumeAnomaly(
		...
		const avgVolume =
			historicalVolumes.reduce((sum, vol) => sum + vol, 0) /
	...
	private detectVolatilityAnomaly(
		...
		const historicalVolatility = this.calculateVolatility(
			prices.slice(-lookbackPeriod - 5, -5),
		);
	...
	private clusterLevels(levels: number[], sensitivity: number): number[] {
		...
		for (let i = 1; i < sortedLevels.length; i++) {
			...
			const clusterMean =
				currentCluster.reduce((sum, val) => sum + val, 0) /
