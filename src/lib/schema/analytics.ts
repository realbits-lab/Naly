...
export const modelPerformance = pgTable(
	...
	(table) => ({
		modelNameIdx: index("idx_model_performance_name").on(
			table.modelName,
...
export const predictionTracking = pgTable(
	"prediction_tracking",
	{
		id: uuid("id").primaryKey().defaultRandom(),
		predictionId: uuid("prediction_id").notNull(),
		actualOutcome: jsonb("actual_outcome"),
		predictionAccuracy: decimal("prediction_accuracy", {
			precision: 3,
			scale: 2,
		}),
		evaluationDate: date("evaluation_date").notNull(),
	...
	(table) => ({
		accuracyIdx: index("idx_prediction_tracking_accuracy").on(
...
export const featureImportance = pgTable(
	...
	(table) => ({
		modelFeatureIdx: index("idx_feature_importance_model").on(
			table.modelName,
			table.modelVersion,
			table.evaluationDate,
		),
...
export const abTestResults = pgTable(
	...
	(table) => ({
		testVariantIdx: index("idx_ab_test_name_variant").on(
			table.testName,
			table.variant,
		),
...
export const dataQualityMetrics = pgTable(
	...
	(table) => ({
		sourceTypeIdx: index("idx_data_quality_source_type").on(
			table.dataSource,
			table.dataType,
		),
		tickerDateIdx: index("idx_data_quality_ticker_date").on(
			table.ticker,
			table.evaluationDate,
		),
