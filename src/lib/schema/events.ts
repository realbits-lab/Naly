import { pgTable, uuid, varchar, timestamp, decimal, jsonb, pgEnum, index } from 'drizzle-orm/pg-core'
...
export const events = pgTable('events', {
  ...
  tickerTimestampIdx: index('idx_events_ticker_timestamp').on(table.ticker, table.timestamp),
  significanceIdx: index('idx_events_significance').on(table.significance, table.timestamp),
...
export const marketDataPoints = pgTable('market_data_points', {
  ...
  tickerTimestampIdx: index('idx_market_data_ticker_timestamp').on(table.ticker, table.timestamp),
  sourceTypeIdx: index('idx_market_data_source_type').on(table.source, table.dataType),
