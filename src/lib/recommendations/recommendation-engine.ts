...
import { UserProfile, AudienceType, InvestmentExperience } from '@/types/user'
...
export interface RecommendationEngine {
  ...
  getPersonalizedContent(userId: string, contentType: ContentType): Promise<PersonalizedContent[]>
...
export class AIRecommendationEngine implements RecommendationEngine {
  ...
  async generateRecommendations(
    ...
    try {
      ...
      recommendations.push(...investmentRecs, ...contentRecs, ...portfolioRecs, ...riskRecs, ...learningRecs)
  ...
  private async generatePortfolioRecommendations(
    ...
    const concentratedPositions = positions.filter((pos: any) => pos.weight > concentrationThreshold)
  ...
  private async generateRiskAlerts(
    ...
    if (context.userProfile.riskTolerance === 'CONSERVATIVE') {
      recommendations.push({
        ...
        description: 'Based on your conservative risk profile, consider reviewing high-volatility positions in your portfolio.',
        confidence: 0.75,
        priority: 'medium',
  ...
  private async generateLearningRecommendations(
    ...
    if (context.userProfile.experienceLevel === 'BEGINNER') {
      recommendations.push({
        ...
        description: 'Learn fundamental investment concepts tailored to your experience level.',
        confidence: 0.9,
        priority: 'medium',
        category: 'Education',
        actionable: true,
  ...
  async getPersonalizedContent(userId: string, contentType: ContentType): Promise<PersonalizedContent[]> {
  ...
  async updateUserProfile(userId: string, preferences: UserPreferences): Promise<void> {
  ...
  private determinePriority(confidence: number, riskLevel?: string): 'low' | 'medium' | 'high' | 'urgent' {
  ...
  private extractTopicsFromInteractions(interactions: UserInteraction[]): string[] {
