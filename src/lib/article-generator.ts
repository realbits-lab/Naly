...
export class ArticleGenerator {
  ...
  private createPrompt(newsArticle: NewsArticle, relatedInfo: RelatedInfo): string {
...
- Ensure all content is original and not copied from the source`
  ...
  private parseGeneratedText(
    ...
    try {
      ...
      return {
        ...
        summary: parsed.summary || 'AI-generated analysis of recent financial news',
        keyPoints: parsed.keyPoints || [],
        marketAnalysis: parsed.marketAnalysis || 'Market analysis not available',
        investmentImplications: parsed.investmentImplications || 'Investment implications under review',
        metadata: {
      ...
      return {
        ...
        content: text,
        summary: 'Analysis of recent financial developments and market implications',
        keyPoints: [],
        marketAnalysis: relatedInfo.marketImpact || 'Market analysis pending',
        investmentImplications: 'Please consult with financial advisors for investment decisions',
        metadata: {
  ...
  private generateMockArticle(
    ...
    return {
      ...
      marketAnalysis: relatedInfo.marketImpact || 'Market impact assessment indicates mixed signals across sectors with potential for increased volatility in the near term.',
      investmentImplications: this.generateInvestmentImplications(newsArticle, relatedInfo),
      metadata: {
  ...
  private createMockContent(newsArticle: NewsArticle, relatedInfo: RelatedInfo): string {
    const sentimentWord = relatedInfo.sentiment === 'positive' ? 'optimistic' : relatedInfo.sentiment === 'negative' ? 'cautious' : 'mixed'
...
*This analysis is based on publicly available information and should not be considered as personalized investment advice. Please consult with qualified financial professionals before making investment decisions.*`
  ...
  private generateInvestmentImplications(newsArticle: NewsArticle, relatedInfo: RelatedInfo): string {
    const implications = {
      'monetary-policy': {
        positive: 'Consider increasing exposure to growth sectors and interest-rate sensitive assets. Monitor bond yields and equity valuations.',
        negative: 'Review defensive positioning and consider shorter-duration fixed income. Monitor credit spreads and risk assets.',
        neutral: 'Maintain balanced allocation while monitoring policy signals. Consider tactical adjustments based on data releases.'
      },
      'technology': {
        positive: 'Technology sector may benefit from continued innovation investment. Consider growth-oriented tech positions.',
        negative: 'Exercise caution with high-valuation tech names. Consider defensive tech positions or sector rotation.',
        neutral: 'Mixed signals suggest selective approach to tech investments. Focus on fundamental strength.'
      },
      'energy': {
        positive: 'Energy sector positioning may be favorable. Consider commodity exposure and energy infrastructure.',
        negative: 'Energy sector headwinds suggest caution. Monitor commodity trends and regulatory developments.',
        neutral: 'Energy sector remains volatile. Consider hedged exposure and monitor supply-demand dynamics.'
      },
      'cryptocurrency': {
        positive: 'Digital asset adoption trends support selective crypto exposure within risk parameters.',
        negative: 'Crypto volatility concerns suggest reduced exposure and careful risk management.',
        neutral: 'Crypto markets remain news-driven. Maintain disciplined approach to digital asset allocation.'
      }
    }

    const categoryKey = newsArticle.category as keyof typeof implications
    const sentimentKey = relatedInfo.sentiment as keyof typeof implications[typeof categoryKey]

    return implications[categoryKey]?.[sentimentKey] ||
