...
export const getVisualizationTypeForData = (data: any[]): string => {
  ...
  // Check for OHLC data
  if (firstItem.open !== undefined && firstItem.high !== undefined &&
  ...
  if (firstItem.type && (firstItem.type === 'BULL_CASE' || firstItem.type === 'BEAR_CASE' || firstItem.type === 'BASE_CASE')) {
    return 'FAN_CHART'
  }
...
export const calculateOptimalDimensions = (type: string, dataSize: number): { width: number; height: number } => {
  const baseWidth = 800
...
export const getColorScheme = (theme: string, seriesCount: number): string[] => {
  const schemes = {
    LIGHT: [
      '#3b82f6', // Blue
      '#10b981', // Green
...
export const validateVisualizationData = (data: any[], type: string): { valid: boolean; issues: string[] } => {
  const issues: string[] = []
    ...
    case 'PROBABILITY_CHART':
      data.forEach((item, index) => {
        if (typeof item.probability !== 'number' || item.probability < 0 || item.probability > 1) {
          issues.push(`Item ${index}: probability must be a number between 0 and 1`)
        }
      })
      break
...
export const formatVisualizationMetadata = (visualization: any) => ({
  ...
  dataPointCount: visualization.data?.datasets?.reduce((sum: number, dataset: any) =>
    sum + (dataset.data?.length || 0), 0) || 0,
  hasInteractivity: Object.values(visualization.interactivity || {}).some(Boolean),
...
export const createVisualizationSummary = (visualizations: any[]) => {
  ...
  const totalDataPoints = visualizations.reduce((sum, viz) =>
    sum + (viz.data?.datasets?.reduce((dataSum: number, dataset: any) =>
      dataSum + (dataset.data?.length || 0), 0) || 0), 0)
  ...
  return {
    ...
    interactiveCount: visualizations.filter(viz =>
      Object.values(viz.interactivity || {}).some(Boolean)).length,
    accessibleCount: visualizations.filter(viz =>
      viz.configuration?.accessibility?.screenReaderSupport).length,
    responsiveCount: visualizations.filter(viz =>
      viz.configuration?.responsive).length
  }
}

export const optimizeVisualizationForPerformance = (visualization: any, targetDataPoints: number = 1000) => {
  ...
  if (optimized.data?.datasets) {
    optimized.data.datasets = optimized.data.datasets.map((dataset: any) => {
      if (dataset.data && dataset.data.length > targetDataPoints) {
        ...
        const sampledData = dataset.data.filter((_: any, index: number) => index % step === 0)
...
export const calculateResponsiveLayout = (visualizationCount: number) => {
  const layouts = {
    ...
    desktop: {
      columns: visualizationCount <= 3 ? visualizationCount :
               visualizationCount <= 6 ? 3 : 4,
      itemHeight: 400
    }
  }
...
export const generateDashboardGrid = (count: number, maxColumns: number = 4) => {
