...
export class ChartGenerator implements VisualizationService {
	...
	private async initializeTemplates(): Promise<void> {
		...
		this.templates.set(VisualizationType.CANDLESTICK_CHART, {
			...
			defaultConfig: this.createDefaultConfig(),
			interactivity: {
				...this.createDefaultInteractivity(),
				zoom: true,
				pan: true,
			},
			accessibility: this.createDefaultAccessibility(),
		...
		return {
			...
			validateData: (data: any[]) => {
				return (
					Array.isArray(data) &&
					data.length > 0 &&
					data.every((d) => d.timestamp && d.value !== undefined && d.dataType)
				);
			},
	...
	private createCandlestickTransformer(): ChartDataTransformer {
		return {
			transformData: (data: any[], config?: any) => {
				...
				const candlestickData = Object.entries(groupedData).map(
					([date, dayData]) => {
		...
		return {
			transformData: (scenarios: any[]) => {
				...
				scenarios.forEach((scenario, index) => {
					const color =
						index === 0 ? "#10b981" : index === 1 ? "#3b82f6" : "#ef4444";
	...
	private generateDashboardInteractions(
		visualizations: Visualization[],
	): InteractionConfig[] {
		...
		visualizations.forEach((viz, index) => {
			if (viz.type === VisualizationType.LINE_CHART) {
				visualizations.forEach((otherViz, otherIndex) => {
					if (
						index !== otherIndex &&
						otherViz.type === VisualizationType.BAR_CHART
					) {
						interactions.push({
