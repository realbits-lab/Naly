...
export const normalizeScenarioProbabilities = (probabilities: number[]): number[] => {
...
export const calculatePredictionRange = (scenarios: any[]): { min: number; max: number; spread: number } => {
...
export const formatPredictionScenarios = (scenarios: any[]) => {
  return scenarios.map(scenario => ({
    ...
    formattedPriceTarget: scenario.priceTarget ?
      `$${scenario.priceTarget.value.toFixed(2)}` : 'N/A',
    scenarioLabel: scenario.type === 'BULL_CASE' ? 'Bull Case' :
                   scenario.type === 'BASE_CASE' ? 'Base Case' :
                   scenario.type === 'BEAR_CASE' ? 'Bear Case' : 'Unknown',
    confidenceLabel: scenario.priceTarget?.confidence ?
      `${Math.round(scenario.priceTarget.confidence * 100)}%` : 'N/A'
  }))
}

export const assessPredictionReliability = (uncertainty: any): 'High' | 'Medium' | 'Low' => {
...
export const calculateModelConsensus = (modelContributions: any[]): number => {
  ...
  const weightedMean = contributions.reduce((sum, contrib, i) => sum + contrib * weights[i], 0) /
                      weights.reduce((sum, w) => sum + w, 0)

  const weightedVariance = contributions.reduce((sum, contrib, i) =>
    sum + weights[i] * Math.pow(contrib - weightedMean, 2), 0) /
    weights.reduce((sum, w) => sum + w, 0)
...
// Validation helpers
export const validatePredictionScenario = (scenario: any): { valid: boolean, issues: string[] } => {
  const issues: string[] = []
  ...
  if (!scenario.priceTarget) {
    ...
    if (typeof scenario.priceTarget.value !== 'number' || scenario.priceTarget.value <= 0) {
      issues.push('Price target value must be a positive number')
    }

    if (typeof scenario.priceTarget.confidence !== 'number' ||
        scenario.priceTarget.confidence < 0 || scenario.priceTarget.confidence > 1) {
...
export const validateEnsemblePrediction = (ensemble: any): { valid: boolean, issues: string[] } => {
  const issues: string[] = []
  ...
  if (!Array.isArray(ensemble.scenarios) || ensemble.scenarios.length === 0) {
    ...
    const totalProbability = ensemble.scenarios.reduce((sum: number, s: any) => sum + (s.probability || 0), 0)
