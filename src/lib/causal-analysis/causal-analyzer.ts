...
import { generateAIText, generateCausalExplanation } from "@/lib/ai";
import { db } from "@/lib/db";
import { analysisResults, causalAnalyses } from "@/lib/schema/events";
...
export class CausalAnalyzer implements CausalAnalysisService {
	...
	async analyzeEvent(event: MarketEvent): Promise<CausalAnalysis> {
		...
		try {
			...
			const hypotheses = await this.generateCausalHypotheses(
				event,
				contextualData,
			);

			// Evaluate hypotheses
			const evaluatedHypotheses = await this.evaluateHypotheses(
				hypotheses,
				event,
			);

			// Identify root cause and contributing factors
			const { rootCause, contributingFactors } =
				this.identifyRootCause(evaluatedHypotheses);
	...
	private async gatherContextualData(
		...
		try {
			...
			console.warn(
				`Failed to gather contextual data for ${event.ticker}:`,
				error,
			);
			return [];
		}
	...
	private async generateCausalHypotheses(
		...
		const priceData = contextualData.filter(
			(d) => d.dataType === "STOCK_PRICE",
		);
		if (priceData.length > 1) {
			const priceHypothesis = this.analyzePriceMovementCausation(
				event,
				priceData,
			);
		...
		if (volumeData.length > 1) {
			const volumeHypothesis = this.analyzeVolumePatternCausation(
				event,
				volumeData,
			);
		...
		const newsData = contextualData.filter(
			(d) => d.dataType === "NEWS_ITEM" || d.dataType === "SENTIMENT_SCORE",
		);
		if (newsData.length > 0) {
			const newsHypothesis = this.analyzeNewsSentimentCausation(
				event,
				newsData,
			);
		...
		const marketPatternHypothesis = await this.analyzeMarketPatternCausation(
			event,
			contextualData,
		);
	...
	private async evaluateHypotheses(
		...
		for (const hypothesis of hypotheses) {
			...
			const factor: CausalFactor = {
				...
				supportingEvidence: hypothesis.supportingEvidence.map(
					this.convertToEvidenceItem,
				),
				temporalRelationship: this.determineTemporalRelationship(
					hypothesis,
					event,
				),
			};
	...
	private async generateAlternativeExplanations(
		...
		for (const hypothesis of hypotheses) {
			if (
				hypothesis.factor !== primaryCause.type &&
	...
	private calculateOverallConfidence(
		...
		if (contributingFactors.length > 0) {
			const avgContributing =
				contributingFactors.reduce((sum, f) => sum + f.confidence, 0) /
	...
	private analyzePriceMovementCausation(
		event: MarketEvent,
		...
		const priceChange =
			Math.abs((prices[prices.length - 1] - prices[0]) / prices[0]) * 100;
	...
	private analyzeVolumePatternCausation(
		event: MarketEvent,
		volumeData: MarketDataPoint[],
	): CausalHypothesis | null {
		if (volumeData.length < 5) return null;

		const volumes = volumeData.map((d) => Number(d.value));
		const avgVolume =
			volumes.slice(0, -2).reduce((sum, v) => sum + v, 0) /
			(volumes.length - 2);
	...
	private analyzeNewsSentimentCausation(
		event: MarketEvent,
		newsData: MarketDataPoint[],
	): CausalHypothesis | null {
		...
		const recentNews = newsData.filter(
			(d) =>
				d.timestamp.getTime() >=
				event.timestamp.getTime() - 24 * 60 * 60 * 1000, // Within 24 hours
		);
	...
	private determineTemporalRelationship(
		...
		const timeDiff =
			...
			hypothesis.supportingEvidence[0]?.timestamp.getTime();
	...
	private assessEvidenceQuality(evidenceChain: EvidenceItem[]): number {
		...
		const avgRelevance =
			evidenceChain.reduce((sum, e) => sum + e.relevanceScore, 0) /
			evidenceChain.length;
		const recencyBonus =
			evidenceChain.filter(
				(e) => Date.now() - e.timestamp.getTime() < 7 * 24 * 60 * 60 * 1000, // Within 7 days
	...
	private assessTemporalConsistency(analysis: CausalAnalysis): number {
		const evidenceTimestamps = analysis.evidenceChain
	...
	private isMethodologyAppropriate(analysis: CausalAnalysis): boolean {
		...
		return (
			analysis.methodology === AnalysisMethod.STATISTICAL_INFERENCE ||
