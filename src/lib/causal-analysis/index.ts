...
export const classifyFactorImpact = (magnitude: number, confidence: number): 'MINIMAL' | 'LOW' | 'MODERATE' | 'HIGH' | 'DECISIVE' => {
...
export const calculateEvidenceStrength = (evidenceItems: any[]): number => {
  ...
  const avgRelevance = evidenceItems.reduce((sum, item) =>
    sum + (item.relevanceScore || 0.5), 0) / evidenceItems.length
...
export const formatCausalAnalysis = (analysis: any) => {
  return {
    ...
    strengthRating: analysis.confidenceScore > 0.8 ? 'Strong' :
                   analysis.confidenceScore > 0.6 ? 'Moderate' :
                   analysis.confidenceScore > 0.4 ? 'Weak' : 'Very Weak'
  }
}

// Analysis validation helpers
export const validateCausalAnalysis = (analysis: any): { valid: boolean, issues: string[] } => {
  const issues: string[] = []
...
export const FACTOR_TYPE_DESCRIPTIONS = {
  EARNINGS_SURPRISE: 'Company earnings results significantly different from analyst expectations',
  NEWS_SENTIMENT: 'Market sentiment shift driven by news coverage or announcements',
  INSIDER_ACTIVITY: 'Trading activity by company insiders affecting market perception',
