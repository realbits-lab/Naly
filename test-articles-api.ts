  ...
  try {
    ...
    if (response.articles.length > 0) {
      ...
      response.articles.forEach((article, index) => {
        ...
        console.log(`   📝 Summary: ${article.summary ? article.summary.substring(0, 100) + '...' : 'N/A'}`);
      ...
      console.log('🌐 Page would be available at: /news-articles (requires auth)');
    } else {
      console.log('⚠️  No articles found in database.');
      console.log('💡 Run: dotenv --file .env.local run npx tsx create-test-article.ts');
    }
