...
describe("Article Generation Integration Test", () => {
	...
	it("should fetch latest news from financial datasets API", async () => {
		...
		try {
			...
			console.log(
				`   🔑 Keywords: ${newsResult.relatedInfo.keywords.join(", ")}`,
			);
			...
			expect(newsResult.relatedInfo.sentiment).toMatch(
			...
			console.error(
				"❌ Failed to fetch news from Financial Datasets API:",
				error,
			);
			throw error;
		}
		...
		try {
			...
			if (!response.ok) {
				...
				if (response.status === 401) {
					console.log(
						"🔓 Authentication required - using mock generation approach...",
					);

					// Instead of calling API directly, test the core functionality
					const { ArticleGenerator } = await import(
					...
					console.log(
						`   📊 Word Count: ${generatedArticle.metadata.wordCount}`,
					);
					console.log(
						`   ⏱️  Reading Time: ${generatedArticle.metadata.readingTime} min`,
					);
					console.log(
						`   📝 Summary: ${generatedArticle.summary.substring(0, 100)}...`,
					);
			...
			console.log(
				`   📊 Word Count: ${apiResult.generatedArticle.metadata.wordCount}`,
			);
			console.log(
				`   ⏱️  Reading Time: ${apiResult.generatedArticle.metadata.readingTime} min`,
			);
	...
	it("should verify article is saved in database", async () => {
		...
		if (!testArticleId) {
			console.log(
				"⚠️  No test article ID available, skipping database verification",
			);
			return;
		}
		...
		try {
			...
			console.log(
				`   📝 Summary: ${savedArticle.summary?.substring(0, 100)}...`,
			);
